...
timeout 2
CALL mvn clean site -Dtest=AllTests -Dbuild.version="1.1" 
timeout 2
call mvn package -Dbuild.version="%APP_VERSION%"
ECHO.
ECHO Executing Java programm ...
java -cp C:\%WS_DIR%\%REPO_NAME%\target\%REPO_NAME%-%APP_VERSION%.jar %MAIN_CLASS% 
::java -cp C:\workspace\Title_Validation_E2E\target\%REPO_NAME%-%APP_VERSION%.jar core.Selenium
GOTO END
